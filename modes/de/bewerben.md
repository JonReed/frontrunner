...
3. SUCHEN       → mit bestehenden Reports unter workspace/reports/evaluations/ abgleichen
...
2. In `workspace/reports/evaluations/` per Grep (case-insensitive) nach dem Firmennamen suchen
...
5. **frontrunner Proof Point**: in "Zusätzliche Informationen" einbauen, falls ein solches Feld existiert
...
1. Status mit dem kanonischen CLI auf "Applied" setzen: `node src/tracker/set-status.mjs <report#> Applied` (die Tabelle in `applications.md` nie von Hand editieren)
2. Block G im Report mit den finalen Antworten aktualisieren
3. Nächsten Schritt vorschlagen: `/frontrunner contacto` für LinkedIn-Outreach an den Personalleiter / Hiring Manager

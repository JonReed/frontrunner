...
A durable bridge between *looking at* the pipeline and *acting on* it. frontrunner
...
(`workspace/applications/agent-inbox.md`) that any tool — this CLI, a dashboard button, a cron job,
...
node src/tracker/agent-inbox.mjs add "draft a follow-up for application #7"
node src/tracker/agent-inbox.mjs add "run a scan and triage anything new"
```

## Inspect / resolve

```bash
node src/tracker/agent-inbox.mjs list            # pending items
node src/tracker/agent-inbox.mjs list --all      # include resolved items
node src/tracker/agent-inbox.mjs resolve 1 --result "scored 4.3 — report 012"
```

`workspace/applications/agent-inbox.md` is user-layer (gitignored). Items look like:
...
1. Read `workspace/applications/agent-inbox.md`. If it doesn't exist or has no unchecked items,
   say so and stop.
...
3. After each, mark it `[x]` and append `→ result: <one line>` — either by hand
   or with `node src/tracker/agent-inbox.mjs resolve <n> --result "..."`.

...
The scanner only surfaces what `workspace/search/portals.yml` `title_filter.positive` matches —
...
("consider adding archetype X and reweighting `workspace/search/portals.yml`
...
- `workspace/profile/cv.md` — the **only** source of evidence for suggestions (required)
- `workspace/profile/profile.yml` — `archetypes` (name / level / fit) for what's already targeted
- `workspace/profile/targeting.md` — target roles, framing, and any deal-breakers the user has recorded
- `workspace/search/portals.yml` — the current `title_filter.positive` (and `negative`) keywords
- Optional: if `workspace/applications/tracker.md` has ≥5 entries progressed beyond
...
- **CV evidence** — 1–2 lines from `workspace/profile/cv.md` **quoted verbatim**. If you cannot
...
2. **Deal-breaker filter.** Never suggest titles that violate the
   deal-breakers recorded in `workspace/profile/targeting.md` (e.g. "no people management"
...
3. **Never invent experience.** Every suggestion must be traceable to quoted
   `workspace/profile/cv.md` lines — the source-of-truth boundary applies to suggestions exactly
   as it does to CV content. Keywords get reformulated, never fabricated. If
   the evidence isn't in `workspace/profile/cv.md`, ask the user; don't stretch a quote to fit.
...
3. Skip keywords that duplicate existing coverage (same dedup rule as above);
   preserve the casing style already used in the user's `workspace/search/portals.yml`.
4. Show the **exact YAML diff** against `workspace/search/portals.yml` `title_filter.positive`
   before touching anything.
5. **Never write to `workspace/search/portals.yml` without explicit user confirmation.**
   "Show me the diff" is not a yes. Silence is not a yes.
6. `workspace/search/portals.yml` (user layer) is **the only file this mode writes by
...
7. **Separately-confirmed exception:** accepted titles can additionally become
   `fit: adjacent` archetypes in `workspace/profile/profile.yml` (an existing schema
   ...
   separate confirmation**; never bundle the `workspace/search/portals.yml` and
   `workspace/profile/profile.yml` writes into one confirmation.

## After the Write

- Suggest `/frontrunner scan` — the wider filter only pays off on the next scan.
- Suggest `upskill` scoped to a Stretch title the user liked, to see the gap
  map between the CV and that next-level target.

## Error Handling

- `workspace/profile/cv.md` missing → stop and point at onboarding (`node doctor.mjs --json`).
  There is no evidence base to suggest from, and inventing one is forbidden.
- `workspace/search/portals.yml` missing, or `title_filter.positive` empty → offer to create it
...
- `workspace/profile/profile.yml` or `workspace/profile/targeting.md` missing → **hard stop**: do not
  ...
  fix-first-then-re-run behavior as a missing `workspace/search/portals.yml` above.
  Deal-breakers live in `workspace/profile/targeting.md` — suggestions generated without

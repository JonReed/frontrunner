...
(`/frontrunner email noshow`) instead.
...
> "No active applications to follow up on. Apply to some roles first with `/frontrunner` and come back when they're aging."

**Calibration cross-reference:** `node src/analysis/funnel-velocity.mjs --summary` reports which in-flight applications sit beyond the typical first-response window (its `waiting` block) — those rows are natural follow-up candidates; feed them into this cadence view rather than treating the wait itself as a verdict (silence past the window is common, not a rejection). When the user reports a status change here ("they replied", "rejected"), route it through `node src/tracker/set-status.mjs <report#> <state>` and pass `--on YYYY-MM-DD` when they name the real event day.
...
- Suggest the user run `/frontrunner contacto {company}` to find the right person first
...
> - Trying a different contact via `/frontrunner contacto`
...
**To:** {email or "No contact found — run `/frontrunner contacto` first"}

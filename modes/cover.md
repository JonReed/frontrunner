...
Works in two modes:
- **Slug mode:** `/frontrunner cover {slug}` — loads the existing evaluation report draft as a starting point
- **Paste mode:** `/frontrunner cover` or JD pasted directly — starts from scratch
...
- If the user named a template, run: `node src/cv/cv-templates.mjs resolve cover "<name>"`
- Otherwise run: `node src/cv/cv-templates.mjs resolve cover` (returns the `cover_letter.template` default, or the base template when unset).
...
Run:
```bash
node src/cv/generate-cover-letter.mjs --payload /tmp/cover-payload-{company-slug}.json
```

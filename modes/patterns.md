...
```bash
node src/analysis/analyze-patterns.mjs
```
...
Run `node src/scan/company-history.mjs --summary` as an additional lens. Zero tokens.

**Hygiene first, always.** The summary leads with any aged-Applied rows that look silent — present that list before any card. Tell the user to confirm real or update via `node src/tracker/set-status.mjs <num> <state> --note "responded <date>"` before drawing any conclusion from the cards below it — a stale tracker row produces the same signal as genuine silence.

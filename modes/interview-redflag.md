...
- `workspace/profile/profile.yml` — User profile (for role/archetype context and, for Step 2c, the candidate's location used to select an applicable interview-law or fair-hiring jurisdiction).
- `templates/protected-grounds.yml` — Jurisdiction-keyed table of protected grounds and fair-hiring do-not-ask topics (for Step 2c only). This is a local data reference, not instruction logic.
...
This step detects interviewer questions touching a topic the candidate's jurisdiction protects or identifies as unsuitable for hiring decisions. It is a **fifth signal inside the existing machinery** — it reuses Step 2's per-session present/absent marking, Step 3's aggregation and scoring, Step 4's warning tiers, and the Step 5 blacklist-suggestion bridge (#1856). It does not create a new verdict system, a new score, or a parallel output pipeline.

**Jurisdiction derivation (same as the #2026 series siblings):** derive the candidate's jurisdiction from their location in `workspace/profile/profile.yml`, then look up the matching row in `templates/protected-grounds.yml`. If the table has no row for the jurisdiction, skip this step entirely — no guessing, no applying another jurisdiction's list. The table is a data reference: every row carries a `legal_basis`, per-ground `legitimate_contexts` where they exist, `sources`, and an `as_of` verification date.
...
- **Single borderline question in a session** = a low-confidence observation. Report it (ground + quote + legal basis) but do **not** mark the signal present for that session. MHLW guidance notes that family questions are often asked to soften the interview atmosphere; that does not establish the interviewer's intent, which must never be inferred from the transcript.
...
> [Render in {language.output}. For Ontario: "this question touches {ground}, a Code ground under {legal basis line from the table row}." For Japan: "this question touches {topic}, an item on MHLW's fair-hiring do-not-ask list under {legal basis line from the table row}." Follow with the verbatim quote and, if one applies, the named legitimate context. Do not describe Japanese guidance topics as statutory protected grounds. Close with a note that whether any specific exchange crossed a legal line depends on context the transcript cannot self-certify, and that this is not legal advice.]
...
```markdown
### Protected-ground / fair-hiring questions ({jurisdiction_name})
...
**Legal context:** [Render in {language.output}: for Ontario, "this question touches {ground}, a Code ground under {legal basis line from the table row}"; for Japan, "this question touches {topic}, an item on MHLW's fair-hiring do-not-ask list under {legal basis line from the table row}". Never call Japanese guidance topics statutory protected grounds, and never say "the interviewer broke the law" / "this was illegal" / "discrimination occurred".]
...
**Evidence strength:** {"Single borderline question — low-confidence observation. Reported, not scored; intent cannot be inferred from the transcript." OR "Cleared the corroboration bar — {repeated probing of the same ground / N distinct grounds} in this session; counts toward the red-flag score above."}

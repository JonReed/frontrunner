...
Run `npm run pipeline:prepare` to build the batch input. The shell runner also
...
4. **For each pending URL**:
   ...
   c. Reserve the next REPORT_NUM atomically: `node src/tracker/reserve-report-num.mjs` (release with `--release {num}` after the worker writes the report; stale sentinels are GC'd automatically)
   d. Execute via Bash:
...
- **4-hour protection window.** Sentinels older than 4h are garbage-collected (`src/tracker/verify-pipeline.mjs` triggers this). Reserve the range immediately before spawning workers, not at the start of a long session. Once a worker writes its real report, that slot is permanently safe — only slow or unstarted slots are at risk after 4h.

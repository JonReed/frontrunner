...
export function lockGate(manifest, root) {
  ...
  switch (d.status) {
    ...
    case 'surface-widened':
      warnSkip(manifest.id, `capability surface expanded since you consented (${[...d.addedHosts, ...d.addedEnv].join(', ')}${manifest.allowsLocalhost ? ', localhost' : ''}) — re-consent: \`node src/plugins/plugins.mjs enable ${manifest.id}\``);
      return { load: false };

...
- `node src/plugins/plugins.mjs run notion export` — push each tracker row (company / role /
  status / score) to the "Applications" database under your Career Ops page.
  Add `--dry-run` to preview without writing.
- `node src/plugins/plugins.mjs run notion search "<query>"` — return Notion records that

...
try {
  ...
  } = await import(pathToFileURL(join(ROOT, 'src/cv/theme-style.mjs')).href);
  ...
  const dir = mkdtempSync(join(tmpdir(), 'frontrunner-theme-'));
  try {
  ...
  if (block.includes('id="frontrunner-dynamic-theme"') && block.includes(':root {') && block.includes('--accent-color: #2563eb;') && block.includes('--font-size: 10pt;')) {
  ...
  if (injected.includes('frontrunner-dynamic-theme') && injected.indexOf('frontrunner-dynamic-theme') < injected.indexOf('</head>') && injected.indexOf('frontrunner-dynamic-theme') > injected.indexOf('<style>')) {
  ...
  if (noHead.startsWith('<style id="frontrunner-dynamic-theme"')) pass('injectThemeStyle prepends the block when there is no </head>');
  ...
  {
    const { injectPrintPageCss } = await import(pathToFileURL(join(ROOT, 'src/cv/generate-pdf.mjs')).href);
    ...
    const overrideIdx = withOverride.indexOf('frontrunner-dynamic-theme'); // the profile's style.margin override
    const pageSetupIdx = withOverride.indexOf('frontrunner-page-setup');   // injectPrintPageCss's @page rule

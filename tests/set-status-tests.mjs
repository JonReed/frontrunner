 ...
 *
 * Tests provision a throwaway tracker via the FRONTRUNNER_TRACKER /
 * FRONTRUNNER_TRACKER_LOCK env overrides (same sandbox pattern as
...
function runSetStatus(args, sandbox, extraEnv = {}) {
  const env = {
    ...
    FRONTRUNNER_TRACKER_LOCK: sandbox.lock,
    ...extraEnv,
  };
  try {
...
function makeSandbox(trackerContent) {
  ...
  // The lock env value must live under tmpdir and use the frontrunner prefix
  // (see trackerLockDirFor) or it is ignored — which would still be safe,
  // just contending on the real default lock.
  const lock = join(dir, 'frontrunner-merge-tracker-test.lock');
...
// ── 14. Lock timeout: structured exit 4, JSON error, no write ──
{
  ...
  const r = runSetStatus(['2', 'Applied', '--json'], sb, { FRONTRUNNER_TRACKER_LOCK_TIMEOUT_MS: '300' });
  let parsed = null;
  try { parsed = JSON.parse(r.stdout); } catch {}
...
// ── 14b. Non-timeout lock failure is NOT reported as lock-timeout ─
{
  ...
  // timeouts.
  const blocker = join(sb.dir, 'frontrunner-merge-tracker-blocker');
  writeFileSync(blocker, 'not a directory');
  const badLock = join(blocker, 'frontrunner-merge-tracker-bad.lock');
...
// ── 15. Orphaned recovery guard does not block stale-lock recovery ─
{
  ...
  const lockDir = join(dir, 'frontrunner-merge-tracker-guardtest.lock');
...
// ── 16. Write failure surfaces as a structured error, not a stack ─
{
  if (process.platform !== 'win32' && process.getuid?.() === 0) {
    ...
    const lock = join(dir, 'frontrunner-merge-tracker-wf.lock');

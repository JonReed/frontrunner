...
try {
  ...
  const page0 = { jobs: Array.from({ length: 50 }, (_, i) => mk(i)), total: 83, page: 0, page_size: 50, total_pages: 2 };
  const page1 = { jobs: [mk(50), mk(51), { title: '', url: 'https://speedrun-talent-network.com/jobs/bad' }], total: 83, page: 1, page_size: 50, total_pages: 2 };
  const calls = [];
  const ctx = {
    fetchJson: async (url) => {
      calls.push(url);
  ...
  const urlsOk = calls.length === 2
    ...
    && calls.every((u) => new URL(u).searchParams.get('source') === 'frontrunner')
  ...
  if (urlsOk && jobs.length === 52 && jobs[0].title === 'Role 0' && jobs[51].title === 'Role 51') {
    pass('fetch() paginates 0-based with source=frontrunner + q passthrough, stops after total_pages, drops invalid rows');
  } else {
    fail(`fetch() = ${JSON.stringify({ calls, count: jobs.length })}`);
  }

  // Explicit server pagination wins over the documented local page size.
  const variableCalls = [];
  const variableCtx = {
    fetchJson: async (url) => {
  ...
  const variable = await provider.fetch({ max_pages: 5 }, variableCtx);
  if (variableCalls.length === 2 && variable.length === 3) {
    pass('fetch() follows explicit total_pages when the server page size differs');
  } else {
    fail(`variable page size = ${JSON.stringify({ calls: variableCalls.length, jobs: variable.length })}`);
  }

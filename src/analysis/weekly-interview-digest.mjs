#!/usr/bin/env node
/**
 * weekly-interview-digest.mjs — Weekly Interview Digest for Frontrunner
 *
 * `interview/debrief` and `interview/practice` already write structured
 * session transcripts to `workspace/interviews/sessions/{company-slug}-{role-slug}-
 * {round}-{YYYY-MM-DD}.md` (schema documented in
 * `workspace/interviews/sessions/README.md`). Nothing aggregated them until now —
 ...
 *
 * Reads:
 *   - workspace/interviews/sessions/*.md — front matter (company/role/round/date/
 *     source) + `<!-- competency: tag[, tag...] -->` annotations
 *   - workspace/interviews/question-bank.md (optional, best-effort) — counts
 ...
 *
 * Run: node src/analysis/weekly-interview-digest.mjs
 *      node src/analysis/weekly-interview-digest.mjs --summary
 *      node src/analysis/weekly-interview-digest.mjs --from 2026-07-13 --to 2026-07-19
 *      node src/analysis/weekly-interview-digest.mjs --self-test
 *
...
const DEFAULT_QUESTION_BANK_PATH = INTERVIEW_QUESTION_BANK_FILE;
...
function printSummary(result) {
  ...
  if (result.metadata.sessionsInRange === 0) {
    const reason = !result.metadata.sessionsDirFound
      ? '(workspace/interviews/sessions/ not found)'
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  const valueFlags = new Set(['--from', '--to']);
  const booleanFlags = new Set(['--summary', '--self-test']);
  for (let i = 0; i < args.length; i++) {
    const arg = args[i];
    if (booleanFlags.has(arg)) continue;
    if (valueFlags.has(arg)) {
      if (args[i + 1] === undefined || args[i + 1].startsWith('-')) {
        console.error(`  ${arg} requires a YYYY-MM-DD value`);
        process.exit(1);
      }
      i += 1;
      continue;
  ...
  let result;
  try {
    result = computeWeeklyDigest({ from, to });
  } catch (err) {

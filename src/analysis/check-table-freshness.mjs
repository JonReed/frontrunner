 ...
 *     months; `--max-age-months` or workspace/profile/profile.yml
 ...
 * fails the run.
 *
 * Run: node src/analysis/check-table-freshness.mjs                    (JSON)
 *      node src/analysis/check-table-freshness.mjs --summary
 *      node src/analysis/check-table-freshness.mjs --max-age-months 6
 *      node src/analysis/check-table-freshness.mjs --today 2026-10-02
 *      node src/analysis/check-table-freshness.mjs --self-test
 */
...
export function validateCliArgs(values) {
  const switches = new Set(['--summary', '--self-test']);
  const valued = new Set(['--max-age-months', '--today']);
...
// arrays (top-level, or under any top-level key), plus one-level object maps,
// where at least one object row carries an `as_of` field. Once a collection
// qualifies as a row-set, EVERY
...
export function extractRows(doc) {
  ...
  if (Array.isArray(doc)) {
    collections.push({
      container: '(top-level)',
      items: doc.map((row, index) => ({ row, index })),
    });
    ...
    for (const [key, value] of Object.entries(doc)) {
      if (Array.isArray(value)) {
        collections.push({
          container: key,
          items: value.map((row, index) => ({ row, index })),
        });
      } else if (isObjectRow(value)) {
        collections.push({
          container: key,
          items: Object.entries(value).map(([index, row]) => ({ row, index })),
        });
      }
    }
  }
  const rows = [];
  for (const { container, items } of collections) {
    if (!items.some(({ row }) => isObjectRow(row) && Object.hasOwn(row, 'as_of'))) continue;
    items.forEach(({ row, index }) => {
      if (!isObjectRow(row)) return;
      rows.push({ row, container, index, missingAsOf: !Object.hasOwn(row, 'as_of') });
    });
  }
  return rows;
}
  ...
  for (const { file, doc } of tables) {
    ...
    for (const { row, container, index, missingAsOf } of rows) {
      const jurisdiction = typeof row.jurisdiction === 'string' && row.jurisdiction.trim()
        ...
        : typeof index === 'string' ? index : `${container}[${index}]`;
      ...
      if (asOf > todayDate) {
        warnings.push({
          type: 'warning', file, jurisdiction, field: 'as_of',
          detail: `as_of ${isoDay(asOf)} is in the future relative to today ${isoDay(todayDate)} — row skipped`,
        });
        continue;
      }
...
function loadConfigMaxAge() {
  if (!existsSync(PROFILE_FILE)) return { months: null, warning: null };
  try {
    ...
    if (months === null) {
      return {
        months: null,
        warning: {
          type: 'warning', file: 'workspace/profile/profile.yml', jurisdiction: null, field: 'table_freshness.max_age_months',
...
function printSummary(result, todayStr, maxAgeMonths) {
  ...
  console.log(`\n${'='.repeat(78)}`);
  console.log('  Table Freshness — Frontrunner');
...
function runSelfTest() {
  ...
  const MAPPED_TABLE = yaml.load([
    'jurisdictions:',
    '  AA-TEST:',
    '    rule: "mapping-shaped table"',
    '    as_of: "2026-06-25"',
    '  BB-TEST:',
    '    rule: "future verification typo"',
    '    as_of: "2026-08-01"',
  ].join('\n'));
  ...
  const tables = [
    ...
    { file: 'mapped-fixture.yml', doc: MAPPED_TABLE },
  ...
  check(result.tablesScanned === 3, `all table shapes discovered, non-table skipped (got ${result.tablesScanned})`);
  ...
  check(extractRows(MAPPED_TABLE).length === 2, 'mapping-under-key shape: as_of rows extracted');
  check(extractRows(NOT_A_TABLE).length === 0, 'file without as_of rows yields no rows');
  check(result.rowsChecked === 7, `7 rows checked (malformed, missing and future rows skipped), got ${result.rowsChecked}`);
  ...
  check(!flagged.has('AA-TEST'), 'fresh mapping row produces no finding');

  // warnings
  check(result.warnings.length === 4, `4 warnings (2 malformed, 1 missing, 1 future), got ${result.warnings.length}`);
  ...
  check(result.warnings.some(w => w.jurisdiction === 'BB-TEST' && w.field === 'as_of' && w.detail.includes('in the future')),
    'future as_of produces a warning and cannot make a row appear fresh');
  ...
  check(validateCliArgs(['--summary', '--today', '2026-07-01']) === undefined,
    'recognized CLI flags pass strict validation');
  check((() => { try { validateCliArgs(['--today']); return false; } catch { return true; } })(),
    'CLI flags that require values fail closed');
  check((() => { try { validateCliArgs(['--bogus']); return false; } catch { return true; } })(),

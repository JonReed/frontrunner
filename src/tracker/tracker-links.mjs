 ...
 * `../reports/evaluations/...` when the tracker is at `workspace/applications/tracker.md`, or
 * `workspace/reports/evaluations/...` when it sits at the repo root.
 *
 * Report files canonically live in `<repoRoot>/workspace/reports/evaluations/`, so we resolve the
 * incoming link to that absolute location regardless of how it was written
 * (root-relative `workspace/reports/evaluations/...` per the TSV convention, or an already-normalized
 * `../reports/evaluations/...`) and recompute the path with path.relative. Idempotent:
 ...
 * @param {string} repoRoot     Absolute repo root where `workspace/reports/evaluations/` lives.
...
export function normalizeReportLink(reportField, trackerDir, repoRoot) {
  return reportField.replace(/\]\(([^)]+)\)/g, (whole, linkPath) => {
    const m = linkPath.match(/^(?:\.\.\/)*(?:workspace\/)?(reports\/.+)$/);
    if (!m) return whole; // not a report path — leave untouched
    const reportAbs = join(repoRoot, 'workspace', m[1]);

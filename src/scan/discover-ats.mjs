#!/usr/bin/env node
/**
 * discover-ats.mjs — Company-list → scannable ATS board resolver for frontrunner
 *
 * Takes a list of companies and resolves each to a scannable ATS board by
 * probing the public JSON APIs frontrunner already supports (Greenhouse, Ashby,
 ...
 *
 * Probing hits live third-party APIs, so honor FRONTRUNNER_PORTALS to point at a
...
function printSummary({ resolved, unresolved, duplicates }) {
  console.log(`\n${'='.repeat(78)}`);
  console.log('  ATS Discovery — frontrunner');

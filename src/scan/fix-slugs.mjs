 ...
 *
 * Usage:
 *   node src/scan/fix-slugs.mjs
 *   node src/scan/fix-slugs.mjs --fix
 *   node src/scan/fix-slugs.mjs --file <path> # dry-run inspection only
 */
...
import { parseAtsSlug, verifyPortalsFile } from './verify-portals.mjs';
...
function parseNameScalar(raw) {
  if (typeof raw !== 'string' || raw.length > 500) return null;
  try {
    const value = yaml.load(`value: ${raw}`)?.value;
...
export function splitCompanyBlocks(text) {
  ...
  const blocks = [];
  const sectionStart = lines.findIndex(line => /^tracked_companies:\s*(?:#.*)?$/.test(line));
  if (sectionStart === -1) return { lines, blocks };
  ...
  for (let i = sectionStart + 1; i < sectionEnd; i++) {
    const match = lines[i].match(NAME_LINE_RE);
    if (match && match[1].length > 0) candidates.push({ line: i, match });
  }
  const itemIndent = candidates.length > 0
    ? Math.min(...candidates.map(({ match }) => match[1].length))
    : null;
  let current = null;
  for (const { line: i, match: m } of candidates) {
    if (m[1].length === itemIndent) {
      const name = parseNameScalar(m[2]);
      if (!name) continue;
      if (current) {
        current.endLine = i;
        blocks.push(current);
      }
      current = { name, indent: m[1], startLine: i, endLine: null };
    }
  }
  if (current) {
...
function applyFix(lines, block, suggested, oldAts, dateStr) {
  ...
  if (careersLine !== -1) {
    lines[careersLine] = `${fieldIndent}careers_url: ${careersUrl}${trailingYamlComment(lines[careersLine])}`;
  } else {
  ...
  if (api) {
    if (apiLine !== -1) {
      lines[apiLine] = `${fieldIndent}api: ${api}${trailingYamlComment(lines[apiLine])}`;
...
export function computeFixes(rawText, results, { dateStr = new Date().toISOString().slice(0, 10) } = {}) {
  ...
  for (const block of blocks) {
    if (blocksByName.has(block.name)) {
      throw new Error(`duplicate tracked company name ${JSON.stringify(block.name)} — refusing ambiguous slug repair`);
    }
    blocksByName.set(block.name, block);
  }
  const config = yaml.load(rawText);
  const companies = Array.isArray(config?.tracked_companies) ? config.tracked_companies : [];
  const companiesByName = new Map();
  for (const company of companies) {
    if (!company || typeof company.name !== 'string') continue;
    if (companiesByName.has(company.name)) {
      throw new Error(`duplicate tracked company name ${JSON.stringify(company.name)} — refusing ambiguous slug repair`);
    }
  ...
  for (const r of results) {
    ...
    const company = companiesByName.get(r.name);
    const current = parseAtsSlug(company?.api) || parseAtsSlug(company?.careers_url);
    const verifiedEu = r.eu === true || String(r.url || '').startsWith('https://api.eu.lever.co/');
    if (!current || current.ats !== r.ats || current.slug !== r.slug || Boolean(current.eu) !== verifiedEu) {
      throw new Error(`portal entry ${JSON.stringify(r.name)} changed after verification — rerun before applying`);
    }
...
async function main() {
  ...
  const allowed = new Set(['--fix', '--apply', '--dry-run', '--file']);
  ...
  const filePath = resolve(fileFlag === -1 ? PORTALS_FILE : args[fileFlag + 1]);
  if (fix && filePath !== resolve(PORTALS_FILE)) {
    throw new Error(`writes are restricted to ${PORTALS_FILE}; --file is dry-run only`);
  }
  ...
  const preview = computeFixes(rawText, results);

  printDiff(preview.fixes, { dryRun });

  if (!dryRun && preview.fixes.length > 0) {
    const providers = await loadProviders(PROVIDERS_DIR);
    let appliedFixes = [];
    await mutateFileLocked(PORTALS_FILE, async currentText => {
      const computed = computeFixes(currentText, results);
      const parsed = yaml.load(computed.text);
      const validation = await validatePortalsConfig(parsed, {
        providerIds: new Set(providers.keys()),
      });
      if (validation.errors.length > 0) {
        const details = validation.errors.map(error => `${error.path}: ${error.message}`).join('; ');
        throw new Error(`repaired portals.yml failed validation: ${details}`);
      }
      appliedFixes = computed.fixes;
      return computed.text;
    });
    console.log(`\nportals.yml updated (${appliedFixes.length} fixed).`);
  } else if (dryRun && preview.fixes.length > 0) {

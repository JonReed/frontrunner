...
import { parseScanHistory, detectReposts } from '../analysis/detect-reposts.mjs';
import { normalizeCompany, resolveTrackerPath } from '../tracker/tracker-utils.mjs';
...
export async function loadStatusLogSource() {
  let mod = null;
  try {
    mod = await import('../analysis/funnel-velocity.mjs');
  } catch {
  ...
  for (const row of Array.isArray(rows) ? rows : []) {
    ...
    if (normalized === 'applied') {
      ...
      facts.push({
        ...
        dateBasis,
        // Real src/tracker/set-status.mjs syntax (it rejects unknown flags, so the
        // instruction must only use flags that exist): the response date is
        // recorded through --note, which appends idempotently.
        clearInstruction: `if they actually responded, node src/tracker/set-status.mjs ${row.num} <state> --note "responded <date>" clears this`,
      });
...
export function renderSummary(result) {
  ...
  if (aged.length > 0) {
    lines.push(`  ${aged.length} aged-Applied row(s) look silent — confirm real or update (node src/tracker/set-status.mjs <num> <state> --note "responded <date>").`);
    lines.push('');
  }
...
async function runSelfTest() {
  ...
  // --- every silent fact has date + clearInstruction containing "set-status" ---
  {
    ...
    check(typeof fact.clearInstruction === 'string' && fact.clearInstruction.includes('set-status'), 'silent fact clearInstruction references src/tracker/set-status.mjs');
  }

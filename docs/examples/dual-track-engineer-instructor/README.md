...
| File | Purpose |
|------|---------|
| `workspace/profile/cv.md` | Fictional dual-track CV (Sam Rivera). Use as structural reference for your own. |
...
## How dual-track changes the rest of frontrunner
...
- Layered (what I recommend): one Professional Summary that names both tracks in the first sentence, then experience entries that include both engineering and teaching bullets per role. Use this when the roles actually combined both. See the `workspace/profile/cv.md` in this folder.
...
### Evaluation reports
When frontrunner evaluates an offer, it should detect which archetype the JD targets and pick the matching salary range, the matching CV emphasis, and the matching STAR stories. With two `fit: primary` entries this mostly just works, but check the `Archetype:` line in the report header. If it picked wrong, the rest of the report is wrong too.

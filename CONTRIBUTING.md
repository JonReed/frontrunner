...
- **Integrations that add new third-party data flows** — sync features or hosted services that push your CV, pipeline, or notes beyond Frontrunner's bounded model-evaluation and tailoring paths. Frontrunner stores persistent job-search data locally; model-backed actions send only the task context to the provider the user selected. Reading *public* job-listing APIs locally is welcome (that's how the built-in providers work), but additional personal-data routing requires prior design and security review.

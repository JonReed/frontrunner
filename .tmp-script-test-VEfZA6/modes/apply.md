...
1. Update status to Applied via the canonical CLI: `node src/tracker/set-status.mjs <report#> Applied` (never hand-edit the table)
2. Seed the follow-up schedule: run `node src/tracker/followup-seed.mjs {num} --json` (where `{num}` is the tracker row number). If the candidate applied on a different day than today, pass `--date YYYY-MM-DD` with the actual submission date. It's idempotent, so re-running is safe.

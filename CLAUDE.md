...
`src/lib/upstream-guard.mjs` enforces this at two levels (a `PreToolUse` hook
and the git `pre-push` hook), but the guard is a backstop, not permission to
stop thinking about it. Read-only review — `git fetch upstream`, `gh pr view`,
`gh pr list` — is always fine.

<!--
This rule is repeated from AGENTS.md deliberately, against the convention
below: the failure it prevents is an agent acting before it has read
AGENTS.md, so the rule has to sit at the entry point to do its job.
-->

<!-- Add other Claude Code-specific guidance here only when it has no AGENTS.md counterpart. -->

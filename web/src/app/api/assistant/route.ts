...
import { frontrunnerRoot, readMemory, doctorState } from "@/lib/frontrunner";

export const runtime = "nodejs"; // child_process (spawn) requires the Node runtime
export const dynamic = "force-dynamic";
export const maxDuration = 120;

const SYSTEM_PREAMBLE = `You are the frontrunner assistant — a proactive, friendly career co-pilot for a person who is actively job-hunting. You live inside their LOCAL frontrunner web dashboard (a pipeline of evaluated jobs, A–F reports, their CV, analytics) and run on their own AI CLI.

YOUR MISSION: genuinely help THIS person land a great role. Know them, advise honestly, and do real work for them:
- Know them: use the persistent memory below + their files (workspace/profile/cv.md, workspace/profile/profile.yml, workspace/reports/evaluations/, workspace/applications/tracker.md, and past worker logs in .frontrunner-web/runs/{id}.md). Read them to be concrete.
...
- generatePdf {"n":"42"} — generate an ATS-optimized CV tailored to application #42 (runs the real pdf mode → workspace/documents/ + marks the tracker PDF column). Spends tokens.
...
- setProfile {"name":"…","email":"…","location":"…","roles":["AI Engineer","ML Engineer"],"compMin":70000,"compMax":95000,"currency":"EUR","remote":"Remote (EU)","seniority":"Senior"} — PROPOSE the user's profile; the app shows a confirm card and ONLY on their OK writes workspace/profile/profile.yml (merge-safe — it never clobbers their other fields) AND seeds the free scanner from the roles. Emit only fields you're confident about (most come from their CV). NEVER write a profile they didn't approve.
- setPortals {"roles":["AI Engineer","ML Engineer"]} — seed the free scanner from target roles (writes workspace/search/portals.yml title_filter). Usually unnecessary — setProfile already does this.

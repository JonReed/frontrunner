...
function readCanonicalMode(): string | null {
  try {
    return fs.readFileSync(path.join(frontrunnerRoot(), "modes", "cv-ingest.md"), "utf8");
  } catch {
    return null;
  }
}
...
export async function POST(req: Request) {
  ...
  try {
    if (ctype.includes("application/json")) {
      ...
      const dir = fs.mkdtempSync(path.join(os.tmpdir(), "frontrunner-cv-"));
  ...
  let child;
  try {
    child = spawn(binPath, args, { cwd: frontrunnerRoot(), env: process.env });
  } catch (e) {

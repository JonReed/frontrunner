...
import { frontrunnerRoot, readMemory } from "@/lib/frontrunner";
...
// The web ORCHESTRATES the real frontrunner engine — it does NOT reimplement it.
...
// (src/tracker/reserve-report-num.mjs → workspace/reports/evaluations/ → workspace/.state/tracker-additions/ → src/tracker/merge-tracker.mjs),
...
Target: ${input}`;
  ...
  if (kind === "pdf") {
    return `You are generating the user's ATS-optimized, TAILORED CV PDF for application #${input}, headless, on their machine. Run the REAL frontrunner "pdf" mode — follow modes/pdf.md EXACTLY (do not improvise a format).
1. Read modes/pdf.md, workspace/profile/cv.md, workspace/profile/profile.yml, and the evaluation report at workspace/reports/evaluations/${input}-*.md (for the JD keywords + analysis).
...
4. Render the PDF: \`node src/cv/generate-pdf.mjs /tmp/cv-{candidate}-{company}.html workspace/documents/cv-{candidate}-{company}-${today}.pdf --format={letter for US/Canada companies, else a4}\`.
5. Update the tracker: in workspace/applications/tracker.md, change the PDF column for row #${input} from ❌ to ✅.
Do not submit anything anywhere.

End with EXACTLY one final line: VERDICT: {5 if the PDF was written, else 1}/5 — {the workspace/documents/ path, ≤12 words}`;
  }
  if (kind === "fix-portal") {
    return `A company's job-portal ATS slug is BROKEN — frontrunner can no longer scan it, so it silently disappears from every future scan. Repair it (headless, on the user's machine):
...
2. Open workspace/search/portals.yml, find the "${input}" entry under tracked_companies, and update its careers_url (and any api/slug field) to the suggested WORKING ATS URL. Change ONLY this one company; preserve all other YAML structure, comments and formatting exactly.
3. Re-run \`node src/scan/verify-portals.mjs\` and confirm "${input}" now shows ✅ live (not ❌).
If NO slug variant resolves, say so clearly and leave workspace/search/portals.yml unchanged. Never touch any other company.
...
End with EXACTLY one final line: VERDICT: {5 if now live, else 1}/5 — {what you changed, ≤12 words}`;
  ...
  return `You are running the OFFICIAL frontrunner job evaluation, HEADLESS, on the user's own machine. Today is ${today}. Run the REAL frontrunner evaluation — do NOT improvise your own scoring.

1. Read modes/oferta.md and follow it EXACTLY (blocks A–F, G posting-legitimacy, and the Machine Summary). Ground the fit in THIS person: read workspace/profile/cv.md, workspace/profile/profile.yml and workspace/profile/targeting.md. Use WebFetch to read the posting (you are headless — Playwright is unavailable, so use WebFetch and mark the report header "Verification: unconfirmed (batch mode)").
...
2. Persist the result CANONICALLY so the web and the CLI share ONE source of truth:
   ...
   b. Write the full report to workspace/reports/evaluations/{num}-{company-slug}-${today}.md  (company-slug = company lowercased, non-alphanumerics → hyphens).
   c. Append ONE row of 9 TAB-separated columns to workspace/.state/tracker-additions/{num}-{company-slug}.tsv, in THIS exact order (real \\t tabs, status BEFORE score):
      {num}\t${today}\t{Company}\t{Role}\t{CanonicalStatus e.g. Evaluated}\t{score}/5\t❌\t[{num}](workspace/reports/evaluations/{num}-{company-slug}-${today}.md)\t{one-line note}
   d. Merge into the tracker: run \`node src/tracker/merge-tracker.mjs\` (it dedupes by company+role+report-num, validates the status, and writes workspace/applications/tracker.md — NEVER edit applications.md by hand).
...
export async function POST(req: Request) {
  ...
  if (required && !fs.existsSync(path.join(frontrunnerRoot(), required))) {
    return new Response(
      JSON.stringify({
        error: `This needs a complete frontrunner checkout (${required}). FRONTRUNNER_ROOT has data only — point it at a full checkout.`,
      }),
  ...
  // hallucinate a fit narrative and still emit a VERDICT. Require workspace/profile/cv.md first.
  if ((kind === "evaluate" || kind === "pdf") && !fs.existsSync(path.join(frontrunnerRoot(), "workspace/profile/cv.md"))) {
    return new Response(
  ...
  // For write-needing kinds, snapshot workspace/reports/evaluations/ so we can verify the worker
  // actually persisted (non-Claude CLIs lack Write auth and silently no-op).
  const reportsDir = path.join(frontrunnerRoot(), "reports");
  const countReports = () => {
    try {
  ...
  const child = spawn(binPath, args, { cwd: frontrunnerRoot(), env: process.env });
    ...
    start(controller) {
      ...
      child.on("close", (code) => {
        ...
        if (!emittedText && !sawError && !cleanExit) {
          ...
          send({ type: "error", msg: "The CLI produced no output — is it installed and authenticated? (frontrunner is best on Claude Code.)" });

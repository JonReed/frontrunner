...
import { frontrunnerRoot, doctorState, readApplications, readInbox, trackerCanDelete } from "@/lib/frontrunner";
...
function lineCount(rel: string, predicate: (l: string) => boolean): number {
  try {
    return fs
      .readFileSync(path.join(frontrunnerRoot(), rel), "utf8")
      .split("\n")
...
function dirCount(rel: string, ext: string): number {
  try {
    return fs.readdirSync(path.join(frontrunnerRoot(), rel)).filter((f) => f.endsWith(ext)).length;
  } catch {
    return 0;
  }
}
...
export async function GET() {
  ...
  const inboxCandidates = lineCount("workspace/search/pipeline.md", (l) => /^\s*-\s*\[[ xX]\]/.test(l));
  ...
  return Response.json({
    ...
    setup: {
      phase: doctor.phase,
      missing: doctor.missing, // system prereq FILENAMES only (workspace/profile/cv.md, workspace/search/portals.yml…)
      hasCv: doctor.hasCv,
    ...
    data: {
      ...
      followupsFile: fs.existsSync(path.join(frontrunnerRoot(), "data", "follow-ups.md")),
    },
    capabilities: {

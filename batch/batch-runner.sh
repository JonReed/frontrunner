...
reserve_report_num_unlocked() {
  ...
  # calling `node src/tracker/reserve-report-num.mjs` directly -- e.g. an interactively
  ...
  local report_num=""
  report_num=$(node "$PROJECT_DIR/src/tracker/reserve-report-num.mjs" 2>/dev/null | tr -d '[:space:]')
...
release_report_num() {
  ...
  node "$PROJECT_DIR/src/tracker/reserve-report-num.mjs" --release "$report_num" >/dev/null 2>&1 || true
}
...
process_offer() {
  ...
  if [[ $exit_code -eq 0 ]]; then
    ...
    # worker's one authoritative final result, not

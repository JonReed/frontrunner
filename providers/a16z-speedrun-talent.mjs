...
//
// Every request carries `source=frontrunner` — the feed's documented optional
...
export default {
  ...
  async fetch(entry, ctx) {
    ...
    for (let page = 0; page < maxPages; page++) {
      const params = new URLSearchParams({ page: String(page), source: 'frontrunner' });
      if (q) params.set('q', q);
      ...
      // Prefer the server's explicit page count. The documented page size is
      // currently 50, but a smaller server-side page must not make us stop
      // early when total_pages says more data remains.

...
export function parseCryptocurrencyJobsRss(xml) {
  ...
  for (let i = 1; i < parts.length && results.length < MAX_ITEMS; i++) {
    try {
      const block = parts[i];
      const rawTitle = decodeFeedText(extractTag(block, 'title'));
      const link = decodeFeedText(extractTag(block, 'link') || extractTag(block, 'guid'));
      ...
      if (parsedUrl.protocol !== 'https:' || parsedUrl.hostname !== 'cryptocurrencyjobs.co') continue;
      ...
      // populate it so scan.mjs's content_filter can run. The board contains
      // remote, hybrid and on-site roles but has no structured location field,
      // so do not invent one from prose.
...
export default {
  ...
  async fetch(_entry, ctx) {
    // Refuse server-side redirects: the feed host is fixed, so one is a signal
    // to fail the board visibly. Transport errors also propagate so a board
    // outage is not misreported as a successful scan with zero openings.
